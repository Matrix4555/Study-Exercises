...
Wine::Wine() : years(0), label(nullptr), PairArray(ArrayInt(), ArrayInt())
{ }

Wine::Wine(const char* l, int y, const int yr[], const int bot[]) : years(y), PairArray(ArrayInt(yr, y), ArrayInt(bot, y))
{
	label = new char[strlen(l) + 1];
	strcpy_s(label, strlen(l) + 1, l);
}

Wine::Wine(const char* l, int y) : years(y), PairArray(ArrayInt(0, y), ArrayInt(0, y))
{
	label = new char[strlen(l) + 1];
	strcpy_s(label, strlen(l) + 1, l);
}
...
void Wine::GetBottles()
{
	...
	PairArray::operator=(PairArray(forF, forS));
}
